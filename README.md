...
- 双方向の新規・更新同期
- 長い日本語パスをGoogle Driveのカスタムプロパティ上限内へUTF-8単位で分割し、旧形式と後方互換で保存
- 同期中は事前検証・現在の処理・完了件数を進捗バーで表示し、完了または失敗を画面に保持
- 両側変更時は競合コピーを作成
- 前回同期記録とHMAC認証付きDrive削除履歴を使う双方向削除同期
- 削除反映時はローカルファイルを`.trash/google-drive-vault-sync/`へ移動し、削除と編集の競合は保留
...
- 削除履歴の全端末受信を証明できないため、Drive上の暗号文を自動で物理削除する機能は未実装です。
- iPhoneではBRATを使ってベータ版を導入します。一般公開後はコミュニティプラグインから導入できる予定です。
- 同期中にObsidianを終了しないでください。
- 同期が途中で失敗しても、完了済みの項目は1件ごとに同期記録へ保存されます。原因を解消して再プレビューすると残りを継続できます。
